...
run.salmon <- function(index, decoys, salmon_path="salmon", kmer, lib_dir,pe_se=c("pe","se"), threads, trme){
  pwd <- getwd()
  if(dir.exists(paste0(pwd,"/quant_out"))==F){
      dir.create(paste0(pwd,"/quant_out"))
    }
dir.create(paste0(pwd,"/",index))
  ...
  system(
    paste(paste0("INDEX_PATH=",paste0(pwd,"/",index),";"),
          ...
          "-o", paste0("temp/",index,"/${fn}_q;"),"done;",
          "mv", paste0(pwd,"/quant_out/temp/",index,"$READS_PATH/*"),paste0(pwd,"/quant_out/ ;"),
          "rm -r temp/"
    )
  )
  }else{
